...
There are geographic features other than city blocks in between the Manhattan locations and the Brooklyn "Di Fara" place.  You're forced to take a bridge, for example, that doesn't make a right angle to the other parts of the route.    

####  *Question 3:*  Which distance metric seems to be most conservative (overstating distance:  Manhattan, Euclidean, Haversine)?
The Manhattan formula overstates the distance the most.  Comparing the chicago outputs for Manhattan and Haversine shows that the Manhattan distance is greater.

####  *Question 4:*  Which Spherical Ellipsoid Distance is most accurate (Haversine, Meeus, or Geo)?  How can you verify that?

"The most precise method, to compute distances, which is Karney’s method, as implemented in the distGeo function" -https://rspatial.org/raster/sphere/2-distance.html
...
w1, w2, and w3 have the same characters - they are anagrams. Building a function to sSorting the strings and check them for equality would find anagrams. 
